...
public class GUT__grand_unified_theory_ extends PApplet {
...
//float k = 992.397463;
///float grav = 6.283473;
float grav = 9.87654321f;
float k = 1.1000001f;
//float mass = 1.5366822E-8;
float mass = 0.0020000117f;
float h = 15%100;
//float dran = 0.7000007;
float dran = 0.20000067f;
...
public void draw() {
 background( 0 );
  for(int i=1;i<gravatons.size();i++) {
    ...
    for(int j=0;j<i/2;j++) {
      ...
      if(sqrt( dr.x * dr.x + dr.y * dr.y ) < h * grav ) {
        set(PApplet.parseInt( A.x.x ), PApplet.parseInt( A.x.y ), color(255,0,0));
        set(PApplet.parseInt( B.x.x ), PApplet.parseInt( B.x.y ), color(0,255,0));
  ...
  text("A = " + (gravaton)gravatons.get(PApplet.parseInt(random(1))),10,105);
...
public void keyPressed() {
  ...
  if (key == 'w') { k += 0.1f; }
  if (key == 's') { k -= 0.1f; }
  if (key == 'e') { mass += 0.002f; }
  if (key == 'd') { mass -= 0.002f; }
  if (key == 'r') { dran += 0.1f; }
  if (key == 'f') { dran += -0.1f; }
}
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#FFFFFF", "GUT__grand_unified_theory_" });
  }
}
